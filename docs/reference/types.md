...
Database record structure for user vaults.
...
Database record structure for encrypted secrets.
...
Database record structure for security audit logs.

```typescript
interface SecurityEvent {
  id: string;
  userId: string;
  eventType: 'password_changed' | 'account_recovered' | 'login_failed' | 'vault_unlocked';
  timestamp: Date;
  details?: string;
  ipAddress?: string;
  userAgent?: string;
...
Default Argon2 configuration values.

```typescript
const defaultArgonConfig: ArgonConfig = {
  time: 3,      // 3 iterations
...
interface EnvironmentConfig {
  development: ArgonConfig;
  production: ArgonConfig;
  testing: ArgonConfig;
}
...
const environmentConfigs: EnvironmentConfig = {
  development: {
    time: 1,
    mem: 16384,  // 16MB - faster for development
    hashLen: 32
  },
  production: {
    time: 4,
    mem: 131072, // 128MB - stronger for production
    hashLen: 32
  },
  testing: {
    time: 1,
    mem: 8192,   // 8MB - fastest for tests
    hashLen: 32
  }
};
```
...
See [Error Reference](/reference/errors.md) for detailed error type definitions.
...
```typescript
function isArgonConfig(obj: any): obj is ArgonConfig {
  return obj &&
    typeof obj.time === 'number' &&
    typeof obj.mem === 'number' &&
    typeof obj.hashLen === 'number';
}

function isAttemptTracker(obj: any): obj is AttemptTracker {
  return obj &&
...
function isMnemonicPhrase(phrase: string): boolean {
...
```typescript
// Using types for better development experience
const params: GenerateEncryptedKeyParams = {
...
const result: GenerateEncryptedKeyResult = await generateEncryptedKey(params);

if (result.success) {
  // TypeScript knows these properties exist
  const { encryptedKey, salt, iv, recoveryPhrase } = result;
...
```typescript
function validateVaultRecord(data: any): data is VaultRecord {
  return data &&
    typeof data.userId === 'string' &&
    typeof data.encryptedKey === 'string' &&
    typeof data.salt === 'string' &&
    typeof data.iv === 'string' &&
    typeof data.encryptedRecoveryKey === 'string' &&
    typeof data.recoverySalt === 'string' &&
    typeof data.recoveryIV === 'string' &&
    data.createdAt instanceof Date;
}

// Usage
const userData = await getUserVault(userId);
if (validateVaultRecord(userData)) {
  // TypeScript now knows userData is VaultRecord
  const result = await decryptGeneratedKey({
    salt: userData.salt,
    iv: userData.iv,
    encryptedKey: userData.encryptedKey,
